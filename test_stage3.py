...
def run(args):
    ...
    params = load_parameters(param_path=f"parameters/params_stage3.json", copy_to_dir=None)
    batch_size = 1
    data_dir = params["data_dir"]

    # test data
    dataset_tests = CollagenDataset(rank=0, data_dir=os.path.join(data_dir, "test"), stage=3, rand_augment=False)
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(dataloader_test, 0):
            ...
            save_path = os.path.join(dirs["raw"], f"{filename}.png")
