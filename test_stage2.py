...
def run(args):
    ...
    dirs = make_directories(root_dir=out_dir, sub_dirs=["stage2"])
    ...
    params = load_parameters(param_path=f"parameters/params_stage2.json", copy_to_dir=None)
    batch_size = 1
    data_dir = params["data_dir"]

    # test data
    dataset_tests = CollagenDataset(rank=0, data_dir=os.path.join(data_dir, "train"), stage=2, rand_augment=False)
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(dataloader_test, 0):
            ...
            image_recon = as_np(image_recon).squeeze()
            image_recon = (image_recon+1)/2

            filename = "{:03d}".format(int(data["filename"][0]))

            save_path = os.path.join(dirs["stage2"], f"{filename}.png")
            save_image(image_recon, save_path)
